...
const SettingsPage = () => {
  ...
  const fetchPlatformConfig = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/platform-config', {
        headers: authHeader()
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleCoordSubmit = async (e) => {
    ...
    try {
      const url = coordEditId 
        ? `http://localhost:5000/api/auth/coordinators/${coordEditId}` 
  ...
  const handleCoordDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/auth/coordinators/${id}`, {
        method: 'DELETE',
        headers: authHeader()
      });
      const data = await res.json();
